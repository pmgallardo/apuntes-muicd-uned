...
Una regla nemotécnica para recordarlas el acrónimo CTFOT formado por las iniciales es usar la palabra CaTFOTo (como si fuera una foto de un gato).
...
La **intensidad** se refiere a la luminosidad.

Es adecuado para indicar variables **cuantitativas** y **ordinales**, ya que una mayor luminosidad se puede interpretar como un valor mayor. No es adecuado para variables categóricas si no se quiere sugerir jerarquía.
...
Se usan en la composición de imágenes como criterio de agrupación de elementos visibles.
...
La frase "*Fueron Por Pan Solo Dos Conocidos Jugadores*" puede ser una regla nmemotécnica para recordar las iniciales de estos principios (FPPSDCJ).
...
- **Juego de objetivos**: la forma en que agrupamos los elementos visuales puede verse influida por estímulos o experiencias percibidas previamente. Es decir, la interpretación actual de una composición no depende solo de lo que vemos en ese momento, sino también del contexto visual anterior.
...
\
**Percepción o interacción.**: Debe decidirse si la visualización solo servirá para comunicar información o si permitirá al usuario explorarla, filtrarla, seleccionarla o ampliarla. Las visualizaciones interactivas ofrecen más posibilidades, pero también requieren una organización más compleja de los datos.

#### Número de variables en la representación )

\
**Número de variables y sus dimensiones**: Hay que determinar cuántas variables se representarán simultáneamente. Pueden utilizarse la posición, el tiempo, el color, la forma o la textura para codificar información, pero incluir demasiadas variables puede dificultar la interpretación. Por ello, deben mostrarse únicamente las relevantes para el objetivo.

#### Número de instancias en la representación )

\
**Número de instancias.** Si existen pocos casos, pueden representarse individualmente. Si el número es elevado, será necesario reducir la información mediante agrupaciones jerárquicas, muestras representativas o medidas estadísticas resumidas, evitando gráficos saturados o lentos de procesar.

#### Tipos de variables en la representación )

\
**Tipo de variables.** La representación debe adecuarse a la naturaleza de las variables: categóricas, ordinales o cuantitativas. Cada tipo de variable se corresponde mejor con determinados canales visuales, como color, posición, tamaño o forma.

`EXAM=(2024J2.3.B)`

La siguiente tabla, obtenida de [IIV], muestra la adecuación de la representación visual dependiendo de los distintos tipos de variables.
...
El acrónimo UGEJBCEDS se puede recordar como "Un Gato Estudiaba Japonés Buscando Comida En Dos Sacos".
...
- **Similaridad:** la repetición de estilos, formas, colores o estructuras permite relacionar elementos y mantener continuidad visual, aunque representen información diferente.
