...
$$E[(Y - \hat{f}(x))^2] = \operatorname{Var}(\varepsilon) + [\operatorname{Bias}(\hat{f}(x))]^2 + \operatorname{Var}(\hat{f}(x))$$
...
Dada la fórmula, el error irreducible es varianza de ese ruido, $\operatorname{Var}(\varepsilon)$.
...
$$
SE_B(\hat{\theta}) = \sqrt{\frac{1}{B-1} \sum_{r=1}^{B} \left(\hat{\theta}^{*r} - \overline{\hat{\theta}^{*}}\right)^2}
$$
...
\operatorname{Var}(\text{promedio}) = \rho \sigma^2 + \frac{1-\rho}{B}\sigma^2
