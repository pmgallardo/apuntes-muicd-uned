...
En él se realiza una introducción a la intensificación o *boosting*, para luego ver los enfoques de AdaBoost y de intensificación de gradiente, junto con las implementaciones de esta última (XGBoost, LightBoost y CatBoost).
...
- Básica
  - [ISL] 8.2.3 "Boosting", donde, en unos pocos párrafos, se explican estos conceptos.
...
La intensificación puede aplicarse tanto a regresión como a clasificación. En este tema se estudia principalmente utilizando árboles de decisión como modelos base.

<!--Diferencia fundamental entre boosting y bagging-->
\
...
\
`EXAM=(2021J2.4,2026J2.6)`

En boosting, cada nuevo árbol se construye de manera secuencial utilizando información de los errores que deja el modelo acumulado para reducir la función de pérdida.

![Diagrama de intensificación](src/apuntes/img/2_boosting_ensemble.png)

En regresión, una forma sencilla de entenderlo es pensar que cada árbol se concentra en la parte de la predicción que todavía falla. El caso concreto en que estos errores son los **residuos** y equivalen al gradiente negativo de una pérdida cuadrática se estudiará en el apartado de intensificación de gradiente.
...
\
En bagging y árboles aleatorios, aumentar mucho el número de árboles suele estabilizar la predicción sin provocar sobreajuste importante por el mero hecho de añadir árboles.

En intensificación, la situación es diferente:
...
- por ello, la intensificación (*boosting*) puede ser más propenso al sobreajuste que bagging.
...
<!--Aprendices débiles-->
\
Un **aprendiz débil** (*weak learner*) es un modelo sencillo que, por sí solo, solo tiene una capacidad predictiva limitada, pero aporta algo de información útil.
...
`EXAM=(2021J2.5,2023J2.5)`

Los aprendices débiles pueden ser:

- árboles pequeños;
- regresiones lineales;
- otros modelos sencillos adecuados al problema.

En intensificación con árboles, son habituales los árboles de poca profundidad, porque cada árbol solo tiene que aportar una corrección parcial.

`EXAM=(2021J2.25,2023J2.4)`

La **asunción de aprendizaje débil** (*weak learner asumption*) establece que los aprendices débiles debe hacerlo ligeramente mejor que el azar.

En clasificación binaria, un clasificador aleatorio tiene error aproximado $0.5$. Por tanto, si $\gamma > 0$, un aprendiz débil debe cumplir:
...
- no se exige que cada modelo individual sea muy bueno;
- basta con que sea un poco mejor que clasificar al azar (error $<0.5$);
- el ensemble puede combinar muchos aprendices débiles para construir un predictor fuerte.

Este concepto está especialmente asociado a la fundamentación de la intensificación, AdaBoost y también de la intensificación del gradiente.

\
La idea principal de AdaBoost es entrenar sucesivos clasificadores prestando cada vez más atención a los ejemplos que los clasificadores anteriores han predicho mal, y combinar después sus decisiones mediante una votación ponderada.
...
\
La **intensificación de gradiente** (***gradient boosting***) es un método de ensemble que construye un modelo sumando modelos sencillos de forma secuencial, normalmente árboles de decisión cortos, de 3 a 6 niveles.

La idea principal es que en cada iteración se añade un árbol que intenta reducir los errores del modelo acumulado según una función de pérdida.
...
\
En el apartado introductorio se vio que intensificación construye árboles secuencialmente para corregir errores anteriores.

La intensificación de gradiente generaliza esa idea:
...
<!--Función de pérdida-->
\
La **función de pérdida** mide cuánto se equivoca el modelo. Intensificación de gradiente intenta reducir progresivamente esa cantidad.
...
\
La intensificación de gradiente puede entenderse como un procedimiento de optimización.
...
\
El **procedimiento** o **algoritmo** general es:
...
<!--Diferencia de intensificación de gradiente respecto a AdaBoost-->
\
`EXAM=(2022SO.4,2024SO.4,2021J2.24)`

**Diferencia de intensificación de gradiente respecto a AdaBoost**:

AdaBoost e intensificación de gradiente son métodos secuenciales, pero su explicación operativa es diferente.

| Método | Qué cambia en cada iteración | Cómo combina modelos | Función de pérdida |
| - | - | - | - |
| AdaBoost | Repondera explícitamente los ejemplos, aumentando la importancia de los mal clasificados | Votación ponderada: cada clasificador débil tiene un peso según su rendimiento | Pérdida exponencial |
| Intensificación de gradiente | Ajusta un nuevo modelo al gradiente negativo o pseudo-residuo de una función de pérdida | Suma aditiva de correcciones, normalmente escaladas por la tasa de aprendizaje | Puede usar distintas funciones de pérdida diferenciables |

En **AdaBoost**:

- cada ejemplo tiene un peso que indica la importancia de clasificarlo correctamente en la ronda actual;
- los ejemplos mal clasificados aumentan su peso;
- cada clasificador débil recibe también un peso en la votación final, según su calidad.

En **intensificación del gradiente**:

- la idea central no es reponderar explícitamente ejemplos mal clasificados;
- se define una función de pérdida;
- cada nuevo modelo intenta corregir el modelo acumulado ajustándose al gradiente negativo o pseudo-residuo de esa pérdida;
- permite usar distintas pérdidas diferenciables, como error cuadrático, pérdida absoluta, Huber o pérdida logística.

**Ideas de examen:**

> AdaBoost asigna pesos a los ejemplos, además de a los clasificadores débiles.

<!---->

> AdaBoost usa la función de pérdida exponencial, mientras que la intensificación de gradiente puede usar cualquier función de pérdida diferenciable.

**Advertencia para el test:**

> Si una pregunta habla de pesos explícitos asignados a ejemplos, ejemplos mal clasificados que ganan importancia o votación ponderada de clasificadores débiles, está describiendo AdaBoost. Si habla de función de pérdida, gradiente negativo o pseudo-residuos, está describiendo intensificación del gradiente.

<!--Tamaño de los árboles-->
\
En intensificación de gradiente no suele buscarse construir árboles muy profundos.
...
<!--Metaparámetros principales de Gradient Boosting-->
\
`EXAM=(2021J2.15)`

En intensificación de gradiente, los **metaparámetros principales** para controlar el modelo son:

| Metaparámetro | Qué controla |
| - | - |
| Número de iteraciones o árboles $M$ | Cuántas correcciones sucesivas se añaden |
| Tasa de aprendizaje $\nu$ | Cuánto aporta cada árbol al modelo acumulado |
| Profundidad máxima o tamaño de los árboles | La complejidad de cada aprendiz débil |
| Fracción de observaciones $\eta$ | Submuestreo usado en Gradient Boosting estocástico |

En la formulación básica del temario, **no se considera necesario especificar el número de hojas terminales** de los árboles. La complejidad de cada árbol se controla normalmente mediante su profundidad o tamaño.

**Idea de examen:**

> Entre los metaparámetros necesarios están la profundidad máxima de los árboles, el número de iteraciones y la tasa de aprendizaje; el número de hojas terminales no aparece como metaparámetro necesario en la formulación preguntada.

<!--Interpretación: importancia de variables-->
\
Un único árbol puede dibujarse e interpretarse fácilmente. Un modelo de intensificación de gradiente es una suma de muchos árboles y no puede interpretarse mirando un único diagrama.
...
- Básica
  ...
  - [BFA] Robert E. Schapire, Yoav Freund. Boosting: Foundations and Algorithms. 2012. <https://direct.mit.edu/books/oa-monograph/5342/BoostingFoundations-and-Algorithms>
