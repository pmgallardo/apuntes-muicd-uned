...
Algunos usos de la reducción de la dimensionalidad son:
...
- Ficha
  ...
  - Versiones de SVD
    - Full: complejidad $O(m x d^2)+O(d^3)$ (mejor cuando d>>n)
    - Aleatorizado
      - estocástico
      - complejidad $O(m x d^2)+O(d^3)$ (mejor cuando n>>d).
...
La **técnica de descomposición de valor singular** o singular value decomposition (SVD) se empea para encontrar los PCs.
...
El ratio de varianza explicada o *explained variance ratio* indica la proporción de varianza que se mantiene en cada PC tras la transformación
...
Escalado multidimensional o *multidimensional scaling* (MDS) reduce la dimensionalidad mientras trata de preserva las distancias entre instancias.

La proyección aleatoria lo hace para muchas dimensiones, pero funciona mal para pocas.
