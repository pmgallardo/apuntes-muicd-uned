<!--
SPDX-FileCopyrightText: 2026 Colaboradores de apuntes_muicd_uned

SPDX-License-Identifier: CC-BY-4.0
-->

# Modelado estadístico de datos

En esta carpeta se incluyen recursos de apoyo educativo para la asignatura de Modelado Estadístico de Datos (MED) del MUICD la UNED, con código 31110018.

Más información en la [ficha web oficial](https://www.uned.es/universidad/inicio/en/estudios/masteres/master-universitario-en-ingenieria-y-ciencia-de-datos/asignaturas.html?codAsignatura=31110018&codTitulacion=311001&idContenido=1).

## Contenido

La carpeta se organiza en la siguiente estructura:

- **[`exam`](exam/)**: Incluye soluciones y enunciados inspirados en exámenes anteriores.
- **[`prac`](prac/)**: Incluye soluciones y enunciados en prácticas anteriores.
- **[`pf`](pf/)**: Incluye preguntas y respuestas inspiradas en cuestiones planteadas en el foro.

Exact collinearity example:

$$\(x_2 = 3 \, x_1\)$$

Near collinearity example:

$$\(x_2 = 3 \approx x_1\)$$

In linear regression:

$$y = X \, \beta + \varepsiln$$

## Comunidades

Existen comunidades no oficiales de alumnos y alumni del máster:

- Comunidad [UNED - MED (Máster Datos)](https://t.me/joinchat/TUr_BZKrWxAyNWM0) en Telegram
- [Carpeta `Modelado estadistico de datos` del Grupo Master Ingenieria y Ciencias de Datos](https://unedo365.sharepoint.com/:f:/s/GrupoMasterIngenieriayCienciasdeDatos/IgDT0Yj-r70SRKB_RQ5Dm-YRAdae9P6J3skb-tQhNmAYaH0) en SharePoint UNED
- Entrada a [MED](https://github.com/adzarei/uned-master-bigdata/tree/master/MED) de curso 2019/2020 en repositorio `uned-master-bigdata` de Adrián José Zapater Reig

## Bibliografía

- Básica
  - JAMES, G., WITTEN, D., HASTIE, T., TIBSHIRANI, R.. An introduction to statistical learning with Applications in R. 2.ª ED. New York: Springer, 2021.  
- Complementaria
  - Hosmer Jr, D. W., Lemeshow, S., Sturdivant, R. X. Applied logistic regression. 2013
  - LETÓN, Emilio, PEDROMINGO, Alejandro. Introducción al análisis de datos en Meta-Análisis. Madrid: Díaz de Santos, 2001.  
  - Peña, D. (2013). Análisis de datos multivariantes. McGraw-Hill España.
  - Peña, D. (2014). Fundamentos de estadística. Alianza Editorial, Madrid.
  - Pepe, M. S. (2003). The statistical evaluation of medical tests for classification and prediction. Oxford University Press.
- Repaso de estadística
  - Ramos, E., Vélez, R. & Hernández, V. (2019). Modelos probabilísticos y optimización. Sanz y Torres, Madrid.  
  - KESSLER, Matthieu. [OCW UCPT Estadística](https://ocw.bib.upct.es/course/view.php?id=81&topic=8)
  - DIEZ, David M, ÇETINKAYA, Mine, BARR, Christopher D. [OpenIntro Statistics](https://www.openintro.org/book/os/). 2019.
