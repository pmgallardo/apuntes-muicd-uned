...
- **Componente conectado**: es un subgrafo maximal en el que existe una trayectoria entre cualquier pareja de nodos. Es maximal porque si se añade otro nodo del grafo se pierde esta propiedad.
...
\
El **alcance** (en inglés, *reach*) mide la capacidad de de un nodo de alcanzar otros nodos de la red mediante trayectorias. Un nodo conectado con gran parte de la red tendrá un alcance elevado; un nodo aislado, que no puede llegar a ningún otro, tendrá alcance nulo.
...
**Pasos** para realizar un análisis de redes sociales, según una metodología de elaboración propia:

1. Definir el **objeto del análisis**.
2. Especificar el **modelo de representación** de los elementos de la red.
3. Identificar las **propiedades** relevantes para el problema.
4. Proponer las **métricas SNA** que cuantifican esas propiedades.
5. Determinar el **método de extracción de datos**.
6. Realizar el **preprocesado**.
7. Construir **atributos** para el análisis.
8. Determinar si hace falta aplicar **aprendizaje automático**, así como el tipo y los algoritmos.
9. Explicar el **criterio de decisión final**

En algunas preguntas de exámenes se pide detallar el SNA a aplicar sobre un problema dado. Puesto que el tiempo del exámen es limitado, bastaría con dedicar un párrafo corto a cada paso.

Métricas sugeridas en función de una propiedad objetivo en un SNA, de elaboración propia:

| Propiedad/constructo | Métrica/indicador |
| - | - |
| Similitud > Equivalencia estructural | Vecinos comunes, sim. Jaccard, sim. coseno |
| Importancia, estatus | Grado de entrada, PageRank, HITS |
