...
Como propuesta de agrupación, se podría construir una red dirigida donde cada estudiante es un nodo y se añade una arista dirigida cada vez que un estudiante responde a otro, estableciendo la dirección de la repuesta. El peso de la arista sería el número de respuestas.
 ...
 A partir de esta red extraería atributos de cada pareja de estudiantes: vecinos comunes, similitud de Jaccard o coseno entre sus patrones de interacción, diferencias de grado de entrada y salida, centralidad, actividad temporal y, si es posible, similitud de contenidos.

Como existen ejemplos pasados etiquetados manualmente por profesores, entrenaría un modelo supervisado que aprenda a predecir si dos estudiantes son similares. El resultado del modelo sería una puntuación de similitud para cada pareja. Con esas puntuaciones se construiría una matriz de similitud y se formarían grupos colocando juntos a estudiantes con alta similitud entre sí.
...
También puede ser útil aprendizaje no supervisado para detectar comunidades implícitas mediante agrupamiento y validar la coherencia estructural de los grupos detectados.
